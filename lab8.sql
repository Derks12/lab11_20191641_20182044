...
-- -----------------------------------------------------
-- Table `bicicentro`.`trabajadores_credenciales`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bicicentro`.`trabajadores_credenciales` (
  `trabajadores_dni` VARCHAR(45) NOT NULL,
  `password_hashed` VARCHAR(64) NOT NULL,
  `email` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`trabajadores_dni`),
  CONSTRAINT `fk_trabajadores_credenciales_trabajadores1`
    FOREIGN KEY (`trabajadores_dni`)
    REFERENCES `bicicentro`.`trabajadores` (`dni`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
